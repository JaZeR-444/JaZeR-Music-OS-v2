...
- OQ-3 - Songs DB Key property: use one 12-option root-note Select: `C`, `C#`, `D`, `E♭`, `E`, `F`, `F#`, `G`, `A♭`, `A`, `B♭`, `B`. Do not add Mode now.
...
|---|---|---|---|---|---|---|
| Name | Title | Required system field, manual entry not required | None | Visible | Notion requires a title. Keep manual naming out of the workflow; use the `Auto Name` formula as the display reference in views. | Phase 2B |
...
| Release | Relation to Releases DB | Optional generally, required for release prep/content tied to a release | Linked Release | Visible when relevant | Connects release work to release readiness. Property can be documented now and activated when Releases DB is built. | Phase 2B spec; active when Releases DB exists |
